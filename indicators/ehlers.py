...
def homodyne_alpha(src: np.ndarray,
    ...
    for i in range(n):
        ...
        # jI = Hilbert of I1 series, where I1[k] = det[k-3]
        # Expanding: _hilbert(I1, i) uses I1[i], I1[i-2], I1[i-4], I1[i-6]
        #   = det[i-3], det[i-5], det[i-7], det[i-9]
        # Compute directly — O(1) instead of O(n) per bar
        _i3 = det[i-3] if i >= 3 else 0.0
        _i5 = det[i-5] if i >= 5 else 0.0
        _i7 = det[i-7] if i >= 7 else 0.0
        _i9 = det[i-9] if i >= 9 else 0.0
        jI[i] = (0.0962 * _i3 + 0.5769 * _i5 - 0.5769 * _i7 - 0.0962 * _i9) * adj
...
def mama_alpha(src: np.ndarray,
    ...
    for i in range(n):
        ...
        # jI = Hilbert of I1 series, where I1[k] = det[k-3]
        # Direct computation — O(1) instead of O(n) per bar
        _i3 = det[i-3] if i >= 3 else 0.0
        _i5 = det[i-5] if i >= 5 else 0.0
        _i7 = det[i-7] if i >= 7 else 0.0
        _i9 = det[i-9] if i >= 9 else 0.0
